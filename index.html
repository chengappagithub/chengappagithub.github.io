...
<meta name="author" content="Desmond Zoambose">
...
<p>
	<spam style="color:dark blue"><strong>
		Introduction
	</strong></spam></br>
A current transformer is a device used to scale down large values of current (for example 100A) to smaller values of 1A or 5A, that can be easily manipulated for the purpose of system protections or metering.</br>
In system protections, the current transformer plays a vital role as it measures current levels under fault conditions and channel these values to the relay for processing. Thus, the CT transformer serves as the eyes of the relay. Depending on the fault current levels, the relay then command the circuit breaker to trip, isolating the faulty portion of the installation from the rest.</br>
In metering systems, the current transformer also plays an important role as it scales down currents of larger magnitudes to smaller magnitude of either 1A or 5A, and then channels this current value to the energy meter for measurements.The energy meter performs an energy calculation base on the current and voltage values it receives within a particular time frame.</br> 
With the above analysis, we can conclude that there exist basically two classes of current transformers which are;<br>
<ul>
<li>Measurement current transformer.</li>
<li>Protection current transformer.</li>
</ul>
The above listed classes of current transformers often poses some confusions as they both literally plays the same principal role (scaling down larger values of currents to smaller values). Perhaps a measurement currrent transformer rated 100/5A will send out a 5A current through it secondary terminal when 100A current passes across its primary, likewise a protection current transformer. The only difference here depends on the loads connected at the receiving ends of the current transformers.The manner in which a relay responses to a signal from a current trasformer is completely different from that of an energy meter. Below we'll discuss some of the reasons why these transformers shouldn't be used for any purpose other from that which they're made for.</br>
<ol>
<li><spam style="color:blue"><strong>
...
<li>5A CT should be used for systems with distances of not more than 10m, while for distances above 10m, 1A CTs should be used. This is done in order to reduce the losses due to the resistance (which strongly depends on the length of the copper cables) of the copper cables leaving the secondary terminals of the CT to the energy meter.</li>
	...
	<spam style="color:blue">
		<strong>Sizing of current transformers for energy meters</strong>
</spam></br>
The signal plate of an a.c generator has the following characteristics:</br>
<ul>
...
To select suitable CTs for the metering system of this generator, we proceed as follows:</br>
<ol>
...
<strong> I= S/(√3U)</strong></br>
=(638×10<sup>3</sup>)/(√3×400)</br>
<strong>I=921A</strong></br>
...
- Determine a suitable position to install the CTs (they could be installed on the power cable leaving the alternator directly in the generator's carbinate or in a separate carbinate housing the power cables.</br>
- We the position of the CT determined, identify a suitable position for the mounting of the energy meter and its accessories.</br>
- Measure the distance between the point where the CTs will be mounted and the point where the energy meter will be mounted and note down the value obtained.</br>
- If the measured distance above is greater than <strong>10m</strong>, then our secondary current should be 1A. And if the reverse is true, then our secondary current should be 5A.</br>
